...
public class FacilityPieces extends SimplePreparableReloadListener<Set<ConfiguredFacilityPiece>> {
    ...
    private static Predicate<ConfiguredFacilityPiece> meetsPiecePositionRequirements(FacilityGenerationContext context, BoundingBox region) {
        ...
        final var surfaceBiomes = context.structureContext.chunkGenerator().getBiomeSource().getBiomesWithin(
    ...
    private static Optional<PlacedFacilityPiece> treeGenerate(FacilityGenerationContext facilityContext,
        ...
        { // Early out for common failures
            ...
            if (center.getY() > facilityContext.structureContext.chunkGenerator().getSeaLevel() - 30) {
                final var pieceBiomes = facilityContext.structureContext.chunkGenerator().biomeSource.getBiomesWithin(
                        center.getX(), center.getY(), center.getZ(), /* Radius */ 2,
                        facilityContext.structureContext.chunkGenerator().climateSampler());
    ...
    private static Optional<PlacedFacilityPiece> tryReplaceRoom(FacilityGenerationContext facilityContext, ConfiguredFacilityPiece requiredPiece,
        ...
        Stream<PlacedFacilityPiece> replacedRoomStream = replaceableRooms.stream().map(replacingPiece -> {
            ...
            var nextStructure = nextPiece.createStructurePiece(facilityContext.structureContext.structureManager(), 0);
    ...
    private static Optional<PlacedFacilityPiece> tryPlaceEntrance(FacilityGenerationContext facilityContext, int genDepth, BlockPos blockPos) {
        ...
        FacilityPieceInstance entrancePiece = entranceNew.getFacilityPiece().createStructurePiece(facilityContext.structureContext.structureManager(), genDepth);
        ...
        for (Direction dir : directions) {
            ...
            int minXminZ = facilityContext.structureContext.chunkGenerator().getBaseHeight(entranceBB.minX() + 1, entranceBB.minZ() + 1, Heightmap.Types.WORLD_SURFACE_WG, facilityContext.structureContext.heightAccessor());
            int minXmaxZ = facilityContext.structureContext.chunkGenerator().getBaseHeight(entranceBB.minX() + 1, entranceBB.maxZ() - 1, Heightmap.Types.WORLD_SURFACE_WG, facilityContext.structureContext.heightAccessor());
            int maxXminZ = facilityContext.structureContext.chunkGenerator().getBaseHeight(entranceBB.maxX() - 1, entranceBB.minZ() + 1, Heightmap.Types.WORLD_SURFACE_WG, facilityContext.structureContext.heightAccessor());
            int maxXmaxZ = facilityContext.structureContext.chunkGenerator().getBaseHeight(entranceBB.maxX() - 1, entranceBB.maxZ() - 1, Heightmap.Types.WORLD_SURFACE_WG, facilityContext.structureContext.heightAccessor());
    ...
    public static Optional<FacilityKeystone> generateFacility(StructurePiecesBuilder builder, PieceGenerator.Context<NoneFeatureConfiguration> context, int genDepth, BoundingBox allowedRegion) {
