...
public class LatexBeeStingerLayer<T extends ChangedEntity, M extends AdvancedHumanoidModel<T>> extends LatexStuckInBodyLayer<T, M> {
    ...
    protected void renderStuckItem(PoseStack p_116584_, MultiBufferSource p_116585_, int p_116586_, Entity p_116587_, float p_116588_, float p_116589_, float p_116590_, float p_116591_) {
        ...
        for(int i = 0; i < 4; ++i) {
            ...
            Matrix4f matrix4f = new JomlConverter().toJoml(posestack$pose.pose());
            Matrix3f matrix3f = new JomlConverter().toJoml(posestack$pose.normal());
    ...
    private static void vertex(VertexConsumer p_116593_, Matrix4f p_116594_, Matrix3f p_116595_, float p_116596_, int p_116597_, float p_116598_, float p_116599_, int p_116600_) {
        p_116593_.vertex(new JomlConverter().toMojang(p_116594_), p_116596_, (float)p_116597_, 0.0F).color(255, 255, 255, 255).uv(p_116598_, p_116599_).overlayCoords(OverlayTexture.NO_OVERLAY).uv2(p_116600_).normal(new JomlConverter().toMojang(p_116595_), 0.0F, 1.0F, 0.0F).endVertex();
    }
}